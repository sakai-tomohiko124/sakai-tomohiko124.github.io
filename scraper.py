...
def run_once() -> int:
    ...
    logging.info("完了 (コード=%s)", final_rc)
    return final_rc
...
def parse_args():
    p = argparse.ArgumentParser(description="scraper2.py と scrape_train.py をまとめて実行・cron登録するツール")
    p.add_argument("--install-cron", action="store_true", help="このスクリプトを毎時実行するcronエントリを登録する")
